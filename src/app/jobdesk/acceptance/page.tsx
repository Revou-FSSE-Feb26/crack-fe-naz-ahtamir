...
export default function JobDeskAcceptancePage() {
  ...
  // ── State untuk List Records ──
  const [records, setRecords] = useState<AcceptanceRecord[]>([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [sortField, setSortField] = useState<"nama" | "jabatan" | "departemen" | "acceptedAt">("acceptedAt");
  ...
  const jabatanList = useMemo(() => {
    ...
    return jobdeskList.filter((j) => j.departemen === departemen).map((j) => j.jabatan);
  }, [departemen]);
  ...
  // ── Effect: Update selected job ──
  useEffect(() => {
    if (jabatan) {
      const found = jobdeskList.find((j) => j.jabatan === jabatan && j.departemen === departemen);
  ...
  // ── Effect: Fetch records (hanya untuk admin) ──
  useEffect(() => {
    if (isAdmin) {
      fetchRecords();
    }
  ...
  const generatePDF = async (data: any) => {
    ...
    container.innerHTML = `
      ...
      <ul>
        ${data.jobdesk.kompetensi.map((item: string) => `<li>${item}</li>`).join("")}
      </ul>
      <h3 style="font-weight:bold; margin-top:16px;">Fungsi Jabatan</h3>
      <ul>
        ${data.jobdesk.fungsi.map((item: string) => `<li>${item}</li>`).join("")}
      </ul>
      <h3 style="font-weight:bold; margin-top:16px;">Tugas, Wewenang, Tanggung Jawab</h3>
      <ul>
        ${data.jobdesk.tugasWewenang.map((item: string) => `<li>${item}</li>`).join("")}
      </ul>
      ...
      <div className="bg-[#231f20] py-[calc(72px+64px)] px-5 md:px-10 border-b-[4px] border-b-[#f15a22]">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-[#c5c0bb] text-lg mt-2">
            Isi data diri Anda, lalu tanda tangani sebagai bukti penerimaan job desk.
          </p>
        </div>
      </div>
      ...
      <section className="py-12 px-5 md:px-10 bg-[#faf9f7]">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-xl shadow-lg p-8 border border-[#d4cfc9]">
            <h2 className="font-barlow-condensed font-bold text-2xl text-[#231f20] mb-6">Form Penerimaan</h2>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="block font-barlow-condensed font-bold text-sm text-[#231f20] mb-1">Nama Lengkap *</label>
                <input
                  type="text"
                  value={nama}
              ...
              <div>
                <label className="block font-barlow-condensed font-bold text-sm text-[#231f20] mb-1">Departemen *</label>
                <select
                  value={departemen}
                  onChange={(e) => {
              ...
              <div>
                <label className="block font-barlow-condensed font-bold text-sm text-[#231f20] mb-1">Jabatan *</label>
                <select
                  value={jabatan}
              ...
              {selectedJob && (
                <div className="bg-[#f2f0ee] p-4 rounded border border-[#d4cfc9] max-h-48 overflow-y-auto text-sm">
                  <h3 className="font-bold text-[#231f20]">{selectedJob.jabatan}</h3>
                  <p className="mt-1"><strong>Uraian:</strong> {selectedJob.uraian}</p>
                  <p><strong>Spesifikasi:</strong> {selectedJob.spesifikasi}</p>
                  <p><strong>Kompetensi:</strong> {selectedJob.kompetensi.slice(0, 3).join(", ")}...</p>
                </div>
              )}
          ...
          {isAdmin && (
            <div className="bg-white rounded-xl shadow-lg p-8 border border-[#d4cfc9]">
              ...
              <div className="flex flex-wrap gap-2 mb-4 text-sm">
                ...
                <button
                  onClick={() => { setSortField("nama"); setSortOrder(sortOrder === "asc" ? "desc" : "asc"); }}
                  className={`px-2 py-1 rounded ${sortField === "nama" ? "bg-[#f15a22] text-white" : "bg-[#f2f0ee]"}`}
                >
                  Nama {sortField === "nama" && (sortOrder === "asc" ? "↑" : "↓")}
                </button>
                <button
                  onClick={() => { setSortField("jabatan"); setSortOrder(sortOrder === "asc" ? "desc" : "asc"); }}
                  className={`px-2 py-1 rounded ${sortField === "jabatan" ? "bg-[#f15a22] text-white" : "bg-[#f2f0ee]"}`}
                >
                  Jabatan {sortField === "jabatan" && (sortOrder === "asc" ? "↑" : "↓")}
                </button>
                <button
                  onClick={() => { setSortField("departemen"); setSortOrder(sortOrder === "asc" ? "desc" : "asc"); }}
                  className={`px-2 py-1 rounded ${sortField === "departemen" ? "bg-[#f15a22] text-white" : "bg-[#f2f0ee]"}`}
                >
                  Departemen {sortField === "departemen" && (sortOrder === "asc" ? "↑" : "↓")}
                </button>
                <button
                  onClick={() => { setSortField("acceptedAt"); setSortOrder(sortOrder === "asc" ? "desc" : "asc"); }}
                  className={`px-2 py-1 rounded ${sortField === "acceptedAt" ? "bg-[#f15a22] text-white" : "bg-[#f2f0ee]"}`}
                >
                  Tanggal {sortField === "acceptedAt" && (sortOrder === "asc" ? "↑" : "↓")}
                </button>
              </div>

              {loadingRecords ? (
                <div className="text-center py-8 text-[#6b6560]">Memuat data...</div>
              ) : filteredRecords.length === 0 ? (
                <div className="text-center py-8 text-[#6b6560]">
                  {searchTerm ? "Tidak ada data yang cocok" : "Belum ada record penerimaan"}
                </div>
              ) : (
                <div className="overflow-x-auto">
                      ...
                      {filteredRecords.map((record) => (
                        <tr key={record.id} className="hover:bg-[#faf9f7]">
                          ...
                          <td className="px-3 py-2">
                            {new Date(record.acceptedAt).toLocaleDateString("id-ID")}
                          </td>
                  ...
                  <div className="text-xs text-[#6b6560] mt-2">
