...
export default function SubSubElement1013Page() {
  const { user, isAuthenticated, isLoading } = useAuth();
  const router = useRouter();

  const isAdmin = user?.role === "admin";
  const isSupervisor = user?.role === "supervisor" || user?.role === "admin";
  const formConfig = getFormConfig("10.1.3");
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/smk3-data?subSubElementId=10.1.3");
    ...
    <div className="min-h-screen bg-[#f1f0ee]">
      <div className="bg-[#231f20] px-6 md:px-10 py-8 border-b-[3px] border-b-[#f15a22]">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-[#8a8580] text-[13px] mt-1.5">10.1.3 Kerahasiaan Catatan K3</p>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[92vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#e5e0db]">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-wide text-[#f15a22] mb-0.5">10.1.3</p>
            ...
            <div className="px-6 py-5">
              <SMK3DataForm
                subSubElementId="10.1.3"
                subSubElementTitle="10.1.3 Kerahasiaan Catatan K3"
